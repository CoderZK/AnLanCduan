...
-(void)drawRect:(CGRect)rect{
    ...
    [[UIColor colorWithRed:0/255.0 green:221/255.0 blue:183/255.0 alpha:1] setFill];//设置填充色
    [[UIColor colorWithRed:0/255.0 green:221/255.0 blue:183/255.0 alpha:1] setStroke];//设置边框色
...
-(instancetype)initWithFrame:(CGRect)frame theMinValue:(float)minValue theMaxValue:(float)maxValue theStep:(float)step theUnit:(NSString *)unit theNum:(NSInteger)betweenNum{
    ...
    if (self) {
        ...
        _triangleColor          = [UIColor whiteColor];//默认橙色
...
-(UITextField *)valueTF{
    if (!_valueTF) {
        ...
        _valueTF.defaultTextAttributes    = @{
                                              ...
                                              NSForegroundColorAttributeName:[UIColor colorWithRed:0/255.0 green:221/255.0 blue:183/255.0 alpha:1]};
...
-(UILabel *)unitLab{
    if (!_unitLab) {
        _unitLab = [[UILabel alloc]initWithFrame:CGRectMake( self.bounds.size.width/2+20 , 10, 60, 40)];
        _unitLab.textColor = [UIColor colorWithRed:0/255.0 green:221/255.0 blue:183/255.0 alpha:1];
    }
    return _unitLab;
}
...
-(void)setRealValue:(float)realValue animated:(BOOL)animated{
    ...
    if (_step == 1) {
        _valueTF.text   =  [NSString stringWithFormat:@"%.0f",_realValue*_step+_minValue];
    }
...
-(void)setDefaultValue:(float)defaultValue animated:(BOOL)animated{
    ...
    if (_step == 1) {
        _valueTF.text   = [NSString stringWithFormat:@"%.0f",defaultValue];
    }
    ...
    if (_scrollByHand) {
        ...
        if  (_step == 1) {
            if (totalValue >= _maxValue) {
                _valueTF.text = [NSString stringWithFormat:@"%.0f",_maxValue];
            }else if(totalValue <= _minValue){
                _valueTF.text = [NSString stringWithFormat:@"%.0f",_minValue];
            }else{
                _valueTF.text = [NSString stringWithFormat:@"%.0f",value*_step +_minValue];
            }
        }else {
            if (totalValue >= _maxValue) {
