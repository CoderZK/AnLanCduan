...
- (void)requestWithUrl:(NSString *)url andDict:(NSMutableDictionary *)dict{
    ...
    [zkRequestTool networkingPOST:url parameters:dict success:^(NSURLSessionDataTask *task, id responseObject) {
        if ([[NSString stringWithFormat:@"%@",responseObject[@"key"]] integerValue] == 1) {
            ...
            [[UIApplication sharedApplication].keyWindow.rootViewController showAlertWithKey:[NSString stringWithFormat:@"%@",responseObject[@"key"]] message:responseObject[@"jkgl1"]];
        }
